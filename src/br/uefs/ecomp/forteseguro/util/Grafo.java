    ...
    public Aresta<T> buscarAresta(Vertice<T> vertice1, Vertice<T> vertice2) {
        for (Aresta<T> auxAresta : this.listaArestas) {
            if (auxAresta.getVertice1().equals(vertice1) && auxAresta.getVertice2().equals(vertice2)) {
                return auxAresta;
            } else if (auxAresta.getVertice1().equals(vertice2) && auxAresta.getVertice2().equals(vertice1)) {
                return auxAresta;
            }
        }
        return null;
...
//        return arestasIncidentes;
        List<Aresta<T>> arestasIncidentes = new ArrayList<>();
        Iterator<Aresta<T>> it = listaArestas.iterator();
        Aresta<T> aresta;
        
        while (it.hasNext()) {
            aresta = it.next();
            if (aresta.getVertice1().equals(nome) || aresta.getVertice2().equals(nome)) {
